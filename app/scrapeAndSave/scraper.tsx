...
export async function scrapeAndStore (page: number) {
  const { data } = await axios.get(
    `https://replicate.com/explore?latest_models_page=${page}`
  );
  const $ = cheerio.load(data);

  const elements = $('#latest-models div.grid div.flex')
    .map((i, el) => {
      // Edit starts here
    ...
    .get()
    .filter(it => it.name && it.runs && it.runs != 'NaN'); //TODO NaN fix
  for (const element of elements) {
    await db.run(
      'INSERT OR REPLACE INTO models(name, runs, url, author, description, lastUpdatedDate) VALUES(?, ?, ?, ?, ?, ?)',
